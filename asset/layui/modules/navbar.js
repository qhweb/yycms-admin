layui.define(['element','admin'], function (exports) {
    ...
    Navbar.prototype.render = function () {
        ...
        if (typeof (_config.elem) !== 'string' && typeof (_config.elem) !== 'object') {
            admin.error('Navbar error: elem参数未定义或设置出错，具体设置格式请参考文档API.');
        }
        var $container;
        ...
        if ($container.length === 0) {
            admin.error('Navbar error:找不到elem参数配置的容器，请检查.');
        }
        if (_config.data === undefined && _config.url === undefined) {
            admin.error('Navbar error:请为Navbar配置数据源.')
        }
                ...
                if (cacheNavbar.navbar === undefined) {
                    admin.req({
                        ...
                        error: function (error) {
                            admin.error('Navbar error:' + error);
                        },
                        complete: function () {
                            _that.config.elem = $container;
                        }
    ...
    Navbar.prototype.on = function (events, callback) {
        ...
        if (typeof (events) !== 'string') {
            admin.error('Navbar error:事件名配置出错，请参考API文档.');
        }
                ...
                for (var j = 0; j < data[i].children.length; j++) {
                    ...
                    if (child !== undefined && child !== null && child.length > 0) {
                        ...
                        for (var t = 0; t < child.length; t++) {
                            ...
                            ulHtml += '<a href="javascript:;" data-url="' + child[t].href + '">';
                            if (child[t].icon !== undefined && child[t].icon !== '') {
                                if (child[t].icon.indexOf('fa-') !== -1) {
                                    ...
                                    ulHtml += '<i class="layui-icon ' + child[t].icon + '" data-icon="' + child[t].icon + '">' + child[t].icon + '</i>';
                                }
                            }
