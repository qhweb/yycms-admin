...
layui.define(['layer','element','form','table'],function(exports){
  ...
  var YYClass = {
    ...
    ,dialog_iframe:function(obj){
          ...
          var Tplurl = $(obj).data('href')||'404',title = $(obj).text(),dialogid = 'dialog'+Math.random().toString(36).substr(2);
          var objRegExp= /^\d+(px|%)$/;
   ...
   * @param {Object} options
   */
  TableClass.prototype.tool = function (btns) {
      var btns = btns||[];
      var toolbar = [
          { event:'delall',name:'批量删除',href:'',class:'layui-btn-danger'}
      ];
      ...
      $.each(toolbar,function(index,btn){
          if (btn.hasOwnProperty('type') && btn.type == 'search'){
              str += '<input class="layui-input layui-input-inline  layui-btn-sm" style="width:150px;height:31px;float:left;margin-right:10px;" name="key" class="'+this.search+'" placeholder="请输入关键字">';
              ...
              str +='<button class="layui-btn layui-btn-sm '+btn.class+'" data-href="'+btn.href+'" lay-event="'+tbboxid+'_'+btn.event+'">'+btn.name+'</button>';
          }
      })
      str +='</div></script>';
    ...
    var that = this;
      ...
      table.on(that.toolbarTable, function(obj){
          ...
          switch(obj.event){
              case that.id+'_search':
                  var data = $(that.boxid).find('.'+that.search).val();
              ...
              case that.id+'_delall':
                  ...
                  break;
              case that.id+'_add':
                  var href = $(this).data('href')||that.config.addurl;
      ...
      table.on(that.toolTable, function(obj){
          ...
          if(obj.event === 'del'){
              ...
              if (DT === 'dialog') {
                  var DW = $(this).data('width') ?  $(this).data('width') : '50%', DH = $(this).data('height') ? $(this).data('height'): '50%';
                  var objRegExp= /^\d+(px|%)$/;
                  DW = objRegExp.test(DW) ? DW : DW+'px';
                  DH = objRegExp.test(DH) ? DH : DH+'px';
                  YYClass.openDialog({
                      title: title,
                      area: [DW, DH],
                      content: '<div id="op"></div>'
                  });
                  ocontainer = YYClass.view('op');
                  ocontainer.render(editurl).done(function(){
                      $("#"+ocontainer.id).scrollTop(0);
                      layui.element.render();
                  });
              }else if(DT === 'prompt'){
      ...
      form.on('switch(switch)', function(obj){
          var trid = $(this).parent().parent().parent().data('index'),posturl = that.config.editsaveurl||that.config.editurl,id=$(this).data('id'),value = obj.elem.checked===true?1:0,fieldname = $(this).attr('name'),data={};
          data[fieldname] = value;
          data[that.config.pk] = id;
          YYClass.req({
              url:posturl,
              type:'post',
              data:data
...
console.log(views);
    ...
    $.ajax({
      ...
      ,error: function(e){
        ...
        if(e.status === 404){
          that.render(layui.cache.base+'template/404');//显示404错误模板
        } else {
          that.render(layui.cache.base+'template/error');//显示error错误模板
        }
